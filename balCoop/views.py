...
class BalCoopApiView(APIView):
    ...
    def post(self, request):
        data_list = request.data  # Assuming the payload is a list of objects

        # Iterate over the list and create each instance individually
        created_instances = []
        for data in data_list:
            # Check if an entry with the same combination exists
            instance_exists = BalCoopModel.objects.filter(
                periodo=data.get('periodo'),
                mes=data.get('mes'),
                entidad_nit=data.get('entidad_nit'),
                puc_codigo=data.get('puc_codigo')
            ).exists()

            if instance_exists:
                # Handle the case when the entry already exists (update or skip)
                # For example, you can choose to update the existing entry:
                existing_instance = BalCoopModel.objects.get(
                    periodo=data.get('periodo'),
                    mes=data.get('mes'),
                    entidad_nit=data.get('entidad_nit'),
                    puc_codigo=data.get('puc_codigo')
                ...
                serializer = BalCoopSerializer(instance=existing_instance, data=data)
            else:
                # Create a new instance if it doesn't exist
                serializer = BalCoopSerializer(data=data)
